...
EndState::EndState(int result) : Manager() {
	...
	pressToStartEntity = new Entity(_grp_UI);
	...
	addEntity(pressToStartEntity, _grp_UI);
	UITextures_[pressToStartEntity] = new Texture(sdlutils().renderer(), PRESS_TO_START_TEXT, sdlutils().fonts().at("ARIAL24"), build_sdlcolor(0x112233ff), build_sdlcolor(0xffffffff));
